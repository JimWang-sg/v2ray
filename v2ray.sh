...
EOF
 ...
 cat >/etc/v2ray/vmess_qr.json <<EOF
vless://${v2ray_id}@${domain}:443?encryption=none&security=tls&type=ws&host=${domain}&path=${_path}#${node_name}
EOF
 else
 cat >/etc/v2ray/vmess_qr.json <<EOF
 {
...
domain_check() {
 ...
 test_domain=$(ping -c 1 -W 2 "$new_domain" 2>/dev/null | awk -F '[()]' '/PING/{print $2; exit}')
 ...
 [[ -z "$test_domain" ]] && test_domain="(ping 失败或无解析结果)"
